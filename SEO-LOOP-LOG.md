...
## Cycle 9 — 2026-08-24: GSC-driven expansion (/menu/, square pizza, West Long Branch) + GEO pass

Fresh API pull (90d): **71 clicks / 2,651 impressions**, 4x month-over-month (14→57 clicks), but
86 non-brand queries earned **zero clicks**. Five-agent lens analysis (query mining, SERP recon,
on-site, GEO/AI, local entity) → 10 ship-now actions, 8 shipped this cycle (commit 0dbde92,
live-verified):

- **/menu/** — full register-fed menu (424 items, 21 category tables), generated by NEW
  scripts/build-menu-page.py in prebuild AFTER the nightly prune; verify-prices now guards its
  rows (595 prices checked, was 152, and the checker now understands same-name twins like the
  two Shrimp Oreganatas). Restaurant schema gained `menu: /menu/`. Target: the 81-impr
  brand+menu query (pos 4.7, 2.5% CTR) owned today by Slice/DoorDash-style titles.
- **/square-pizza-long-branch/** — homepage was already pos 6.8 on "square pizza long branch"
  with no dedicated page on that SERP; prices via price_of (Grandma 26.00 / Sicilian 27.04 /
  Drunken Grandma 27.04 / Sicilian Pan Pie 29.12). Plus a 15th homepage FAQ (seo.ts + JSON-LD
  in lockstep) linking it.
- **/pizza-delivery-west-long-branch/** — the separate borough, in-zone at the owner's $3 flat
  fee (deliveryZones.ts, 2026-07-31); disambiguates WLB vs West End; Monmouth University angle.
  Target: ~33 impr across 6 queries at pos 31-42 with no owning page.
- **Pier Village page re-aimed** at visit intent ("pizza pier village" family, homepage 5.6 vs
  page 32.8) — title/h1/dek/first section; delivery content kept.
- **Homepage**: meta description now menu-led ("full Long Branch menu with prices…"); the
  21-phrase keywords tag DELETED (keyword stuffing = -10% AI visibility, Princeton GEO);
  **Restaurantji corrected 4.6/86 → 4.4/90** (re-verified against their JSON-LD 8/24) in
  reviews.ts, About tile, all landing heroes, llms.txt. WebPage dateModified refreshed.
- **FAQ internal links**: FAQS entries carry an optional link; 8 answers now anchor to their
  deep pages (GF, vegan, catering, delivery, menu, late-night, VIP, square). FAQPage JSON-LD
  stays plain text; parity checked 15/15 byte-identical.
- **llms.txt**: new ## Pages directory (all 15 URLs), delivery-fee table ($3/$5 tiers), 3 new
  Q&As (square prices, WLB delivery, kids parties), typo fix, Last updated bumped.
- **Index hygiene**: /privacy-policy/ + /sms-terms/ noindexed and dropped from the sitemap
  (privacy page was ranking pos 44 for a Sea Bright brand query). Sitemap = 15 URLs; IndexNow
  200 for all 15.

**Deliberately NOT shipped**: homepage <title> change (queued for Jorge per cycle-2 precedent —
proposed: "Gigi's NY Style Pizza & Restaurant | Long Branch Menu & Delivery"); fonts self-host
(A9 — app uses Playfair 600 but only a 700 file exists; would visibly thicken menu-item names;
needs a 600 woff2 + metric overrides + visual diff); sameAs disentangle (needs Jorge's answer on
whose IG/FB the handles are); named AI-bot robots groups (already allow-all via *).

**Queued for Jorge/Tommy** (full detail in the 8/24 session): B1 GBP order-link reclaim + Google
review count + name decision; B2 title sign-off; B3 Request Indexing on the 4 zero-impression
URLs + the 3 new pages (token is readonly — UI clicks or webmasters-scope re-auth); B4 hours
sweep at the Slice source + claim Yelp + flag the gigisnystylepizzarestaurant.com clone; B5
IG/FB ownership answer; B6 Google-review push + TripAdvisor listing; B7 real dish photos.
