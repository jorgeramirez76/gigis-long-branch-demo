    ...
    <section id="fan-favorites" aria-label="Fan favorites" className="scroll-mt-20 bg-[var(--color-page)] py-8 md:py-12">
      <div className="container-x">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div><span className="eyebrow">The good stuff</span><h2 className="mt-2 text-3xl md:text-4xl">Your next favorite bite.</h2></div>
          ...
          {FAVORITES.map(f => {
            ...
            return (
              <article key={f.menuName} className="flex flex-col overflow-hidden rounded-2xl border border-[var(--color-line)] bg-[var(--color-panel)]">
                <picture>
                  <source type="image/webp" srcSet={f.webp} sizes="(min-width: 768px) 46vw, 100vw" />
                  <img src={f.src} alt={f.alt} loading="lazy" decoding="async" className="aspect-[16/10] w-full object-cover" />
                </picture>
                <div className="flex flex-1 flex-col p-4">
                  ...
                  {item && category && isOrderable(item) ? (
                    <button type="button" onClick={() => configureItem(item, category.id)} className="btn-primary mt-auto w-full text-sm" aria-label={`Customize & add ${f.name}`}>Customize &amp; add</button>
