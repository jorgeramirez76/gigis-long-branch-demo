...
type Confirmation = { orderId?: string; paid: boolean; total: number; discount?: number; fulfillment: Fulfillment; routingIssue?: boolean; duplicate?: boolean; units: number; vipEligible?: boolean; message?: string; vipJoin?: { status: "verify_sent" | "already_member"; email?: string; pollId?: string | null; ttlHours?: number } };
...
export function Checkout({ onClose }: { onClose: () => void }) {
  ...
  // A percent-off code is worth that share of every pizza (toppings included) in the cart —
  // again the server's own function (src/lib/pizzaPercent.ts).
  ...
  async function sendOrder(cardToken?: string, onDispatched?: () => void) {
    ...
    try {
      ...
      setConfirmed({
        ...
        // What the checkout opt-in produced server-side. Shape-checked: anything else
        // falls back to the inline form.
        vipJoin:
          data.vipJoin && (data.vipJoin.status === "verify_sent" || data.vipJoin.status === "already_member")
            ? data.vipJoin
            : undefined,
      });
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
            ...
            <div className="rounded-xl border border-[var(--color-gold,#c89441)]/50 bg-[var(--color-page)] px-4 py-3 text-sm text-[var(--color-copy)]">
              <div className="flex items-center justify-between gap-3">
                <span>
                  ...
                  {promo.kind === "bogo_pizza" ? "buy one pizza, get one free" : promo.kind === "pct_pizza" ? `${promo.percentOff}% off every pizza, toppings included` : `free ${FREE_PIE_ITEM}`}
              ...
              {!promoQualifies && (
                <p className="mt-1.5 text-xs text-[var(--color-action-text)]">
                  {promo.kind === "bogo_pizza"
                    ...
                    : promo.kind === "pct_pizza"
                      ? `Add a pizza from the Pizza menu and ${promo.percentOff}% comes off it here.`
        ...
        <div className="rounded-2xl bg-[var(--color-panel)] p-4 shadow-[var(--shadow-sm)]">
          ...
          <dl className="mt-3 space-y-1 border-t border-[var(--color-line)] pt-3 text-sm">
            ...
            {promoDiscount > 0 && promo && <Row label={promo.kind === "bogo_pizza" ? `Buy one, get one free (${promo.code})` : promo.kind === "pct_pizza" ? `${promo.percentOff}% off pizza (${promo.code})` : `VIP free pie (${promo.code})`} value={`−${money(promoDiscount)}`} />}
